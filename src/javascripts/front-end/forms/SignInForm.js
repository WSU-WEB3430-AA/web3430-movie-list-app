      ...
      fetch('/api/users/signin', {
        ...
        if(!response.ok){
          switch(response.status){
            case 401: throw Error("Invalid credentials")
            case 404: throw Error("User not found")
            case 500: throw Error("Server error")
          }
        } 
        return response.text()
      })
      .then((data) => {
        toast.success("Successfully signed in.", {autoClose: 3000, onClose: () => {
